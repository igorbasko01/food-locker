...
Step 1 also happens on its own. `release-merge.yml` runs at 01:00 UTC every Friday and squash-merges the open release PR, so a week's worth of merged features ships without anyone doing anything. The same workflow has a **Run workflow** button for shipping immediately, which works from the GitHub mobile app.
